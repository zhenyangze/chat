...
class Events
{
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        if( !!trim($message) ) {
            ...
            if(isset($role) && $role === 'visitor'){
                if($type === 'login') {
                    ...
                    if (isset($group)) {
                        ...
                        if ($kfIsOnline) {
                            $clientList = Gateway::getClientSessionsByGroup($group);
                            if ($clientList) {
                                $s_c_g = array();
                    ...
                    if ($kefNoUse) {
                        ...
                        if ($kefuNum > 0) {
                            //原则:平均分配访客给客服
                            $kefuList = Gateway::getClientSessionsByGroup("kf_".$relation);//获取当前客服系统的客服信息
                            $c_n = array();//'客服id'=>'接待量'
                            foreach ($kefuList as $key => $value) {
                                $c_n[$value['uid']] = 0;
                                $clientList = Gateway::getClientSessionsByGroup($value['uid']);//客服接待列表
                                if ($clientList) {
                                    $s_c = array();
                                    foreach ($clientList as $k => $v) {
                                        $s_c[$v['uid']] = $k;
            ...
            }else if(isset($role) && $role === 'worker'){
                if($type === 'login') {
                    ...
                    if(Gateway::getClientCountByGroup($uid)){
                        ...
                        $client_id_s = Gateway::getClientSessionsByGroup($uid);//获取在线访客列表
                        $client_uid_s = array(); //
                ...
                if($type === 'login') {
                    ...
                    //获取在线客服
                    $online_s['kf_online_s'] = Gateway::getClientSessionsByGroup("kf_".$relation);//获取在线访客列表
                    $online_s['client_wait_online_s'] = Gateway::getClientSessionsByGroup("wait_".$relation);//获取在线访客等待列表
                    $online_s['client_chat_online_s'] = Gateway::getClientSessionsByGroup("chat_".$relation);//获取在线访客咨询列表
                    $online_uid_s = array();
